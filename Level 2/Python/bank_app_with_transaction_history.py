class BankApp:
    ...
    def Customer_Database(self):
        ...
        mycon = sql.connect(host = '127.0.0.1', user = 'root', password = 'Adebimpe@01', database = 'bank2_db') #create database
        ...
        mycursor.execute('CREATE TABLE Customer_Table (id INT(4) PRIMARY KEY AUTO_INCREMENT, email VARCHAR(25), password VARCHAR(25), account_no FLOAT(10))')
    ...
    def PayBills(self):
        if self.balance > 0:
            network = input('Enter network:')
            phone_number = input('Enter phone number: ')
            self.nepabill = int(input('How much airtime do you want to buy: '))
            self.Deduct()
